<script>
  ...
  export let triggerValueRe = 0.8;
  export let triggerValueIter = 1;
  export let triggerValueZoomPi = 1;
  let realLimit = triggerValueZoomPi;

  $: if (triggerValueZoomPi !== undefined) {
    console.log("zoom = " + triggerValueZoomPi)
    realLimit = Math.PI/triggerValueZoomPi;
    computeData();
  }
  ...
  function computeData() {
    ...
    const step = (realLimit * 2) / sizeTest;
    const offset = -1 * realLimit;
